# 编辑时间:2025/8/8 18:34
import os.path
import sys

import src.util.copy_util as cu
import src.util.monitor as monitor
from src.util.log_config import log

def print_help():
    """显示帮助信息"""
    print("文件同步工具")
    print("用法: syncFile.exe [选项] [源目录路径] [目标路径]")
    print("选项:")
    print("  -h, --help    显示此帮助信息")
    print("  -v, --version 显示版本信息")
    print("\n例:")
    print("syncFile.exe -h")
    print("syncFile.exe source_path dest_path")
    print("syncFile.exe dest_path")
    print("\n如果未指定目录路径，默认监控当前目录为源目录路径")

def print_version():
    """显示版本信息"""
    print("文件监控工具 v1.0")

def get_effective_path(source):
    if not os.path.isabs(source):
        abs_path = os.path.abspath(source)
        return abs_path
    return source

if __name__ == "__main__":
    source = ""
    dest = ""
    # 处理命令行参数
    if len(sys.argv) > 1:
        if sys.argv[1] in ["-h", "--help"]:
            print_help()
            sys.exit(0)
        elif sys.argv[1] in ["-v", "--version"]:
            print_version()
            sys.exit(0)
        else:
            if len(sys.argv) == 2:
                source = sys.argv[1]
                dest = sys.argv[2]
            else:
                dest = sys.argv[1]
            if not dest:
                log.info("目标文件夹路径不能为空")
    else:
        source = "."  # 当前目录
    source = get_effective_path(source)
    # 程序一运行会先进行文件的比对复制
    log.info(f"源文件目录:{source}")
    log.info(f"目标目录:{dest}")
    files = cu.compare_files(source, dest)
    if len(files[0]):
        log.info(f"需要同步的文件:{'无' if files[0] == [] else files[0]}")
    # 自动重启机制：如果程序崩溃，等待几秒后重新启动
    while True:
        try:
            monitor.run_monitor(source, dest, files[2])
            break  # 如果正常退出（如用户Ctrl+C），则终止循环
        except Exception as e:
            log.warning(f"程序异常，将在 5 秒后重启...{e}")
            time.sleep(5)  # 等待5秒后重启

# if __name__ == '__main__':
#     source = "D:\华为云盘\学习笔记"
#     dest = "F:\mdWeb\docs\md"
#     log.info(f"源文件目录:{source}")
#     log.info(f"目标目录:{dest}")
#     files = cu.compare_files(source, dest)
#     log.info(f"需要同步的文件:{'无' if files[0] == [] else files[0]}")